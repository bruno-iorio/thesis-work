...
def get_target(X,Y): 
    ...
    text_target_previous  = [[-1] + i[:-2] for i in X]

    emotion_input = [i[:-1] for i in Y]
    emotion_target = [i[1:] for i in Y]
    emotion_target_previous = [[-1] + i[:-2] for i in Y]
    ...
    for i in range(len(text_target)):
        for j in range(len(text_target[i])):
            ...
            if text_target[i][j] == 0:
                emotion_target[i][j] = -1
                text_target[i][j] = -1
    for i in range(len(emotion_target)):
        for j in range(len(emotion_target[i])):
            if emotion_target[i][j] == 0:
                emotion_target[i][j] = -1

    for i in range(len(text_target_previous)):
        for j in range(len(text_target_previous[i])):
            if text_target_previous[i][j] == 2:
                emotion_target_previous[i][j] == -1
            if text_target_previous[i][j] == 0:
                emotion_target_previous[i][j] = -1
                text_target_previous[i][j] = -1

    for i in range(len(emotion_target)):
        for j in range(len(emotion_target[i])):
            if emotion_target_previous[i][j] == 0:
                emotion_target_previous[i][j] = -1

    return text_input, text_target, text_target_previous, emotion_input, emotion_target, emotion_target_previous
