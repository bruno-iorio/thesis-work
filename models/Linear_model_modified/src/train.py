...
def inference(model,loader,device,batch_size,weights=None):
    ...
    for batch in loader:
        ...
        hidden = torch.zeros((batch['texts'].size()[0],300)).to(device)
        loss = 0
        for t in range(batch['texts'].size()[1]):
            ...
            pt, _ ,  pe, _ , hidden = model.infer(inp_token,inp_emotion,hidden)
    ...
    return sum(losses)/len(losses),preds,trues,words,pred_words

## train functions
def train_batch(model, batch, optimizer,lr,device,batch_size ,weights=None,max_norm = 1.0,scheduler = None): ## train for each batch (set to size 1 in this case)
    loss = 0
    ...
    hidden = torch.zeros((batch['texts'].size()[0],300), requires_grad=True).to(device)
    losses = []
    valids1 = (batch['target_emotions'] != -1).float()
    valids2 = (batch['target_texts'] != -1).float()
    for t in range(batch['texts'].size()[1]): ## TODO: Test if it works with 5 instead of 55, to run faster
        ...
        target_token = batch['target_texts'][:, t]
        target_emotion = batch['target_emotions'][:, t]
        prev_token = batch['target_texts_previous'][:, t]
        prev_emotion = batch['target_emotions_previous'][:, t]
        
        pt, prev_pt, pe, prev_pe, hidden = model.forward(inp_token, inp_emotion, hidden)
        loss1 = 0.7 * loss_emotions(pe, target_emotion)
        loss2 = 0.3 * loss_words(pt, target_token)
        loss3 = 0.1 * loss_emotions(prev_pe,prev_emotion)
        loss4 = 0.1 * loss_words(prev_pt, prev_token)
        ...
        if torch.isnan(loss2).any() or torch.isinf(loss2).any():
            continue
        if torch.isnan(loss3).any() or torch.isinf(loss3).any():
            continue
        if torch.isnan(loss4).any() or torch.isinf(loss4).any():
            continue
...
def train(model, train_loader, epochs, device, n_total ,batch_size ,lr=0.08,compute_preds = False):
    ...
    for epoch in range(epochs):
        ...
        for it, batch in enumerate(train_loader): 
            batch = {          
                        ...
                        'target_texts' : batch['target_texts'].to(device),
                     'target_emotions' : batch['target_emotions'].to(device),
            'target_emotions_previous' : batch['target_emotions_previous'].to(device),
               'target_texts_previous' : batch['target_texts_previous'].to(device)
            }
            loss_, ln = train_batch(model,batch,optimizer,lr,device,batch_size,weights=weights,max_norm=max_norm,scheduler=scheduler)
            l += ln
            losses.append(loss_)
